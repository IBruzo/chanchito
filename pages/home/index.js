      ...
      <Box boxShadow='xl' borderRadius="lg" m={10} p={4} bg={bgColor}>
        <VStack spacing="1">
          <Box w={400}>
            ...
            <Progress h={6} borderRadius="md" hasStripe colorScheme='green' value={(currAmount/goalAmount)*100}></Progress>
          </Box>
        </VStack>
      </Box>
      </Link>
        ...
        <>
          <VStack spacing={4} h='200vh' bgGradient="linear(45deg, cyan.100, cyan.500)">
            ...
            {piggyList.map((item, index) => (
              <InfoBox key={index} piggyOwner={item.piggyOwner} currAmount={item.currentAmount} goalAmount={item.goalAmount} bgColor='gray.100'>
              </InfoBox>
    ))}
            <Button
              size="lg"
